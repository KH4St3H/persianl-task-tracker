    ...
    <div className="grid gap-8 sm:grid-cols-2">
      <section className="space-y-3">
        ...
        <ul className="divide-y divide-border">
          {projects.map((p) => (
            <li key={p.id} className="flex items-center gap-2 py-2 text-[15px]">
              <input
                type="color"
                defaultValue={p.color}
                onBlur={(e) =>
                  e.target.value !== p.color &&
                  start(() => updateProject(p.id, p.name, e.target.value))
                }
              ...
              <input
                ...
                onBlur={(e) =>
                  ...
                  start(() => updateProject(p.id, e.target.value, p.color))
                }
              ...
              <Button
                ...
                onClick={() =>
                  confirm(`Delete project "${p.name}"? Tasks are kept.`) &&
          ...
          {!projects.length && (
            <li className="py-2 text-[15px] text-muted-foreground">
              No projects yet. Add one to group tasks by area of life.
            </li>
          )}
        </ul>
      </section>
        ...
        <ul className="flex flex-wrap gap-2">
          ...
          {!tags.length && (
            <li className="text-[15px] text-muted-foreground">
              No tags yet. Tags cut across projects, like #errand or #call.
            </li>
          )}
        </ul>
      </section>
